...
class MemoryManager:
    ...
    async def _save_to_db(self, user_id: str, memory: UserMemory) -> None:
        ...
        try:
            ...
            # Handle both sync and async collection methods by testing the actual method
            try:
                # Try calling update_one and check if it returns a coroutine
                result = col.update_one({"_id": user_id}, {"$set": memory.to_dict()}, upsert=True)
                if asyncio.iscoroutine(result):
                    # It's async (Motor), await it
                    await result
                # If it's sync (pymongo), it's already done
            except Exception:
                # Fallback to thread executor for sync calls
                await asyncio.to_thread(col.update_one, {"_id": user_id}, {"$set": memory.to_dict()}, upsert=True)
    ...
    async def _load_from_db(self, user_id: str) -> Optional[UserMemory]:
        ...
        try:
            ...
            # Handle both sync and async collection methods more robustly
            data = None
            try:
                # Try calling find_one
                ...
                # If the first method failed, try with thread executor
                try:
                    data = await asyncio.to_thread(col.find_one, {"_id": user_id})
                except Exception as second_error:
                    logger.debug(f"Both database query methods failed for user {user_id}: {first_error}, {second_error}")
                    return None
            
            # Ensure data is valid before trying to access it
