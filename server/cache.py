...
class CacheManager:
    ...
    async def set(self, key: str, value: Any, expire: Optional[int] = None, ttl: Optional[int] = None) -> None:
    ...
    async def mark_message_processed(self, message_id: str, ttl: int = 3600) -> None:
        """Mark a message as processed with TTL (default 1 hour)"""
        key = f"processed_msg:{message_id}"
        await self.set(key, int(time.time()), ttl=ttl)
    
    async def is_message_processing(self, message_id: str) -> bool:
        """Check if a message is currently being processed"""
        key = f"processing_msg:{message_id}"
        result = await self.get(key)
        return result is not None
    
    async def mark_message_processing(self, message_id: str, ttl: int = 300) -> None:
        """Mark a message as currently being processed (default 5 min TTL)"""
        key = f"processing_msg:{message_id}"
        await self.set(key, int(time.time()), ttl=ttl)
    
    async def unmark_message_processing(self, message_id: str) -> None:
        """Remove processing flag for a message"""
        key = f"processing_msg:{message_id}"
