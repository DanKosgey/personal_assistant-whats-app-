...
Supports both environment variable and JSON file configuration for API keys.
"""
...
class ProfileManagerCLI:
    ...
    async def run(self):
        ...
        print()
        print("Configuration: Supports both environment variables and JSON file (config/api_keys.json)")
        print()
        ...
        while True:
            ...
            print("11. Show configuration info")
            print("12. Exit")
    ...
    async def show_config_info(self):
        ...
        for var in env_vars:
            value = os.getenv(var, "")
            if value:
                print(f"  {var}: {'*' * len(value) if len(value) > 4 else value}")
            else:
                print(f"  {var}: Not set")
