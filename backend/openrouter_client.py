...
class OpenRouterClient:
        ...
        try:
            ...
            if config_path.exists():
                with open(config_path, 'r') as f:
                    config = json.load(f)
                    keys = config.get("openrouter_keys", [])
                    if keys:
                        logging.info(f"Loaded {len(keys)} OpenRouter keys from JSON config")
            else:
                logging.warning(f"API keys config file not found at {config_path}")
        except Exception as e:
            logging.error(f"Error loading API keys from JSON: {e}")
        return keys
    ...
    def load_keys() -> List[str]:
        ...
        keys = OpenRouterClient._load_keys_from_env()
        if keys:
            logging.info(f"Loaded {len(keys)} OpenRouter keys from environment variables")
            return keys
        
        # Fallback to JSON file
        keys = OpenRouterClient._load_keys_from_json()
        return keys
    ...
    def __init__(
        ...
        # Load keys from environment or JSON if not provided
        if keys is None:
            keys = self.load_keys()
